class PlatingCertificate < VarlandPdf
    ...
    def drawTable
        #Use 'span' so that the table flows to the next page without any gaps.
        move_down(_i(4.75))
        span(_i(10.5), :position => :center) do
            #Fill shaded area
            fill_color 'cccccc'
            ...
            stroke_line [_i(0), _i(4.75)], [_i(10.5), _i(4.75)]

            #Draw Vertical Lines
            [0.0, 10.5].each do |x|
                stroke_line [_i(x), _i(4.75)], [_i(x), _i(3.75)]
            end
            [0.6, 1.35, 2.1, 2.85, 3.85, 5.25, 7.1, 9.7].each do |x|
                stroke_line [_i(x), _i(4.0)], [_i(x), _i(3.75)]
            end
            ...
            table(certificate_data, :width => (10.5*72)) do
                ...
                style(row(-1).columns(0...-1), :borders => [:bottom, :left, :right])
            end  
        end 
    end
    ...
    def insertFooter       
        ...
        stroke_line [_i(1.75), _i(0.65)], [_i(4.5), _i(0.65)]
    end
...
#Old code... Get Toby's approval before removing.
=begin
    def draw_platingCert
        ...
        bounding_box [_i(0.25), _i(5.3)], width: _i(10.5), height: _i(4.75) do
            ...
            #COLUMN HEADERS
            certificate_data = [
                ["S.O. #", "S.O. DATE", "POUNDS", "PIECES", "CONTAINERS", "YOUR PO #", "PART DESCRIPTION", "PROCESS SPECIFICATION",  "STATUS"]
            ]
            ...
            #Insert data into the table
            move_cursor_to _i(3.7)
            table(certificate_data, :header => true, :width => (10.5*72)) do
                style(row(0...-1).columns(0...-1), :borders => [:left, :right])
                style(columns(0), :width => (0.6*72), :align => :center)
                style(columns(1), :width => (0.75*72), :align => :center)
                style(columns(2), :width => (0.75*72), :align => :right)
